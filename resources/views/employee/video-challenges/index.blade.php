...
@push('scripts')
<script>
    // Store division data for modals
    const challengeData = @json($challenges->map(function($challenge) {
        return [
            'id' => $challenge->id,
    ...
    function openDivisionModal(challengeIndex, divisionIndex) {
        const data = challengeData[challengeIndex];
        if (!data) return;

        const div = data.divisions[divisionIndex];
        if (!div) return;
        ...
        const submitted = div.employees.filter(e => e.submitted);
        const pending = div.employees.filter(e => !e.submitted);
        ...
        statsDiv.innerHTML = `
            ...
            <div><span class="text-gray-900 font-bold">${div.employees.length}</span> <span class="text-gray-500 text-sm">Total</span></div>
        `;
