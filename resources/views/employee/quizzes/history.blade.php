                            ...
                            @foreach($attempts as $attempt)
                                <tr class="hover:bg-gray-50 transition">
                                    ...
                                    <td class="hidden sm:table-cell px-3 sm:px-6 py-3 sm:py-4">
                                        <span class="px-2 py-0.5 sm:px-2.5 sm:py-1 bg-indigo-100 text-indigo-700 text-[10px] sm:text-xs rounded-full">
                                    ...
                                    <td class="px-3 sm:px-6 py-3 sm:py-4">
                                        @if($attempt->status === 'completed')
                                            <a href="{{ route('employee.quizzes.result', $attempt->id) }}"
                                               class="text-indigo-600 hover:text-indigo-800 font-medium text-xs sm:text-sm">
                                                View Result →
                                            </a>
        ...
        @else
            <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-8 sm:p-16 text-center">
                ...
                <a href="{{ route('employee.quizzes.join') }}"
                   class="inline-flex items-center px-4 sm:px-6 py-2.5 sm:py-3 bg-gradient-to-r from-indigo-600 to-purple-600 text-white rounded-xl hover:shadow-lg transition-all duration-300 font-bold text-sm sm:text-base">
