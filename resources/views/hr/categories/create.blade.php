    ...
    <div class="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
            <div class="px-4 sm:px-8 py-4 sm:py-6 bg-gradient-to-r from-indigo-600 to-purple-600">
                <h2 class="text-xl sm:text-2xl font-bold text-white">Create Quiz Category</h2>
                <p class="text-indigo-100 text-sm mt-1">Add a new category to organize quizzes</p>
            </div>

            <form action="{{ route('hr.categories.store') }}" method="POST" class="p-4 sm:p-8 space-y-4 sm:space-y-6">
                @csrf
                ...
                <div>
                    ...
                    <input type="text" name="name" id="name" value="{{ old('name') }}" required
                        ...
                        placeholder="e.g., Grammar, Vocabulary, Business English">
                    @error('name')
