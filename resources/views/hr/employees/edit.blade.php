...
<div class="max-w-3xl">
    <div class="hr-card overflow-hidden">
        <form action="{{ route('hr.employees.update', $employee) }}" method="POST" class="p-6 sm:p-8 space-y-5">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <textarea name="address" id="address" rows="3" class="hr-input">{{ old('address', $employee->address) }}</textarea>
                @error('address')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="flex flex-col sm:flex-row justify-end gap-3 pt-4 border-t border-slate-100">
                ...
                <button type="submit" class="hr-btn-primary">Update Employee</button>
            </div>
        </form>
    </div>
</div>
