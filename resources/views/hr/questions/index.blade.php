...
<div class="py-8">
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
        ...
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8">
            ...
            <div class="flex flex-col sm:flex-row gap-3 mt-4 sm:mt-0">
                <a href="{{ route('hr.quizzes.questions.create', $quiz) }}" 
                   class="inline-flex items-center justify-center px-6 py-3 bg-gradient-to-r from-indigo-600 to-purple-600 text-white rounded-xl hover:shadow-lg transition-all duration-300 font-medium">
        ...
        {{-- Quiz Info Card (dengan Total Points) --}}
        <div class="bg-gradient-to-r from-indigo-600 to-purple-600 rounded-2xl p-6 mb-8 text-white">
            <div class="grid grid-cols-2 md:grid-cols-5 gap-4">
                ...
                <div>
                    ...
                    <p class="text-2xl font-bold">{{ $quiz->questions->sum('points') ?? 0 }}/100</p>
                </div>
                <div>
        ...
        @if(session('show_import_preview') && session('imported_questions'))
            ...
            <div class="bg-white rounded-2xl shadow-sm border-2 border-green-200 overflow-hidden mb-8">
                <div class="px-6 py-4 bg-gradient-to-r from-green-600 to-emerald-600 text-white">
                    <div class="flex items-center justify-between">
                        <div>
                            ...
                            <p class="text-green-100 text-sm mt-1">Review dan edit sebelum menyimpan ke database</p>
                        </div>
                            ...
                            <div class="flex-1">
                                ...
                                @else
                                    <div class="bg-green-50 rounded-xl p-4 ml-4 border border-green-200">
                                        ...
                                        <p class="text-green-700 whitespace-pre-wrap">{{ $question->correct_answer }}</p>
                                    </div>
                                @endif
                                ...
                                @if($question->explanation)
                                    <div class="bg-blue-50 rounded-xl p-4 mt-3 border border-blue-200">
                                        ...
                                        <p class="text-blue-700 whitespace-pre-wrap">{{ $question->explanation }}</p>
                                    </div>
                                @endif
                            </div>
        ...
        {{-- ============================================== --}}
        {{-- IMPORT PREVIEW SECTION (Form Review) --}}
        {{-- ============================================== --}}
        @if(session('show_import_preview') && session('imported_questions'))
            @php 
            ...
            <div id="importPreview" class="mt-8 bg-white rounded-2xl shadow-lg border-2 border-green-200 overflow-hidden">
                <div class="px-8 py-6 bg-gradient-to-r from-green-600 to-emerald-600 text-white">
                    ...
                    <p class="text-green-100 mt-1">Review {{ $total }} soal yang terdeteksi dari PDF</p>
                </div>

                <form action="{{ route('hr.quizzes.questions.import.confirm', $quiz) }}" method="POST" class="p-8">
                    @csrf

                    {{-- Alert Info --}}
                    <div class="mb-4 p-4 bg-amber-50 border border-amber-200 rounded-xl flex items-start">
                        ...
                        <div class="text-sm text-amber-800">
                            <p class="font-semibold mb-1">Review & Edit Sebelum Menyimpan</p>
                            <p>Periksa setiap soal di bawah. Anda dapat mengedit teks, pilihan jawaban, atau jawaban benar. Uncheck soal yang tidak ingin diimport.</p>
                        </div>
                    </div>

                    {{-- Info Auto Points --}}
                    <div class="mb-6 p-4 bg-indigo-50 border border-indigo-200 rounded-xl flex items-start">
                        ...
                        <div class="text-sm text-indigo-800">
                            <p class="font-semibold mb-1">💯 Auto-Calculate Points (Max 100)</p>
                            <p>Sistem akan otomatis membagi total <strong>100 poin</strong> ke {{ $total }} soal yang dipilih. 
                            Anda dapat mengubah poin per soal secara manual jika diperlukan.</p>
                            <p class="mt-2 text-xs">
                                <strong>Distribusi otomatis:</strong> 
                                @if($remainder > 0)
                    ...
                    <div class="space-y-4 max-h-[600px] overflow-y-auto pr-2">
                        @foreach($importedQuestions as $index => $question)
                            @php
                                // Hitung poin otomatis untuk soal ini
                            ...
                            <div class="border border-gray-200 rounded-xl p-5 hover:shadow-md transition bg-white">
                                <div class="flex items-start justify-between mb-3">
                                    <div class="flex items-center space-x-3">
                                        ...
                                        <span class="w-8 h-8 bg-gradient-to-br from-green-500 to-emerald-600 rounded-lg flex items-center justify-center text-white font-bold text-sm">
                                            {{ $question['order_number'] }}
                                        </span>
                                        <select name="questions[{{ $index }}][question_type]" 
                                                class="px-3 py-1 bg-green-100 text-green-700 text-xs font-semibold rounded-full border-0 focus:ring-2 focus:ring-green-500">
                                    ...
                                    <div class="flex items-center space-x-2">
                                        ...
                                        <input type="number" 
                                               ...
                                               value="{{ $autoPoints }}" 
                                               min="1" 
                                               class="w-20 px-3 py-1 border rounded-lg text-sm focus:ring-2 focus:ring-green-500"
                                               title="Poin otomatis: {{ $autoPoints }}. Bisa diubah manual.">
                                    </div>
                                </div>
                                ...
                                <textarea name="questions[{{ $index }}][question_text]" rows="2" 
                                          class="w-full px-4 py-2 border border-gray-300 rounded-lg mb-3 focus:ring-2 focus:ring-green-500 focus:border-transparent">{{ $question['question_text'] }}</textarea>

                                @if(!empty($question['options']))
                                    <div class="bg-gray-50 rounded-lg p-3 mb-3 space-y-2">
                                        <p class="text-xs font-semibold text-gray-600 mb-2">Pilihan Jawaban:</p>
                                        @foreach($question['options'] as $optIndex => $option)
                                            <div class="flex items-center space-x-2">
                                                <span class="w-6 h-6 bg-green-100 rounded-full flex items-center justify-center text-xs font-bold text-green-700">
                                                    {{ chr(65 + $optIndex) }}
                                                </span>
                                                <input type="text" name="questions[{{ $index }}][options][{{ $optIndex }}]" 
                                                       value="{{ $option }}" class="flex-1 px-3 py-1 border rounded-lg text-sm focus:ring-2 focus:ring-green-500">
                                            </div>
                                        @endforeach
                                    </div>
                                @endif
                                ...
                                <div class="mb-2">
                                    <label class="text-xs font-semibold text-gray-600 flex items-center">
                                        <svg class="w-4 h-4 mr-1 text-green-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"/>
                                        </svg>
                                        Jawaban Benar:
                                        @if($question['question_type'] === 'essay')
                                            <span class="ml-2 text-xs text-amber-600 font-normal italic">
                                                (Essay dinilai manual oleh HRD, jawaban tersimpan sebagai referensi)
                                            </span>
                                        @endif
                                    </label>
                                    
                                    @if($question['question_type'] === 'essay')
                                        {{-- ✅ Untuk Essay: Tampilkan "-" saja, tapi simpan nilai asli di hidden input --}}
                                        <div class="w-full px-3 py-2 border border-gray-300 bg-gray-100 rounded-lg text-sm text-gray-500 italic">
                                            - <span class="text-xs">(Jawaban essay dinilai manual)</span>
                                        </div>
                                        <input type="hidden" 
                                            name="questions[{{ $index }}][correct_answer]" 
                                            value="{{ $question['correct_answer'] }}">
                                    @else
                                        {{-- Untuk tipe soal lain: tetap bisa diedit --}}
                                        <textarea name="questions[{{ $index }}][correct_answer]" 
                                                rows="{{ $question['question_type'] === 'short_answer' ? 1 : 2 }}"
                                                class="w-full px-3 py-2 border border-green-300 bg-green-50 rounded-lg text-sm mt-1 focus:ring-2 focus:ring-green-500">{{ $question['correct_answer'] }}</textarea>
                                    @endif
                                </div>
                                ...
                                <div>
                                    ...
                                    <input type="text" name="questions[{{ $index }}][explanation]" 
                                           ...
                                           class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm mt-1 focus:ring-2 focus:ring-green-500">
                                </div>
                            </div>
                        @endforeach
                    </div>
                    ...
                    <div class="flex justify-between items-center pt-6 mt-6 border-t">
                        <a href="{{ route('hr.quizzes.questions.import.cancel', $quiz) }}" 
                           ...
                           class="px-6 py-3 border border-gray-300 text-gray-700 rounded-xl hover:bg-red-50 hover:border-red-300 hover:text-red-600 transition font-medium">
                            <svg class="w-4 h-4 inline mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        <button type="submit"
                                class="px-6 py-3 bg-gradient-to-r from-green-600 to-emerald-600 text-white rounded-xl hover:shadow-lg transition-all duration-300 font-medium flex items-center">
                            <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
@push('scripts')
<script>
    // Auto-scroll to import preview if exists
