    ...
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
        <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
            <div class="p-6">
                ...
                <form action="{{ route('hr.quizzes.questions.update', [$quiz, $question]) }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')
                    ...
                    <div id="optionsContainer" class="mb-4" style="{{ $question->question_type === 'multiple_choice' ? '' : 'display: none;' }}">
                        ...
                        <div id="optionsList">
                            ...
                            @for($i = 0; $i < 4; $i++)
                                <div class="flex items-center mb-2">
                                    <span class="mr-2 text-sm font-semibold">{{ chr(65 + $i) }}</span>
                    ...
                    <div class="mb-4">
                        ...
                        <textarea name="explanation" id="explanation" rows="2"
                            class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">{{ old('explanation', $question->explanation) }}</textarea>
                        @error('explanation')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
