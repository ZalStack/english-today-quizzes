...
<script>
    ...
    function showDivision(index) {
        ...
        submitted.concat(pending).forEach(function(emp) {
            ...
            div.className = 'flex flex-col sm:flex-row sm:items-center justify-between px-3 sm:px-5 py-2 sm:py-3' + (isSubmitted ? ' bg-green-50/50' : '');
            ...
            left.innerHTML = '<div class="w-7 h-7 sm:w-9 sm:h-9 bg-gradient-to-br ' + bg + ' rounded-lg flex items-center justify-center text-white font-bold text-xs sm:text-sm shrink-0">' + initial + '</div>' +
                '<div class="min-w-0"><p class="font-semibold text-gray-900 text-xs sm:text-sm truncate">' + escapeHtml(emp.name) + '</p>' +
                '<p class="text-[10px] sm:text-xs text-gray-500 truncate">' + escapeHtml(emp.email) + '</p></div>';
